...
import { MissionLogType, missionLogs } from '@/data/mission-logs'
import { styles } from '@/styles/logs-styles'

export default function LogsScreen() {
  const messageLogs = missionLogs.filter((log) => log.type === 'message').length
  const taskLogs = missionLogs.filter((log) => log.type === 'task').length
  const systemLogs = missionLogs.filter((log) => log.type === 'system').length
  const healthLogs = missionLogs.filter((log) => log.type === 'health').length
    ...
    <SafeAreaView edges={['top']} style={styles.container}>
      ...
      >
        <View style={styles.header}>
          <View>
            <Text style={styles.title}>Log da missão</Text>
          ...
          <View style={styles.statusBadge}>
            <Ionicons name="document-text-outline" size={15} color={colors.orange} />
        ...
        >
          <View style={styles.heroIcon}>
            <Ionicons name="time-outline" size={26} color={colors.orange} />
          </View>

          <Text style={styles.heroLabel}>Rastreabilidade operacional</Text>

          <Text style={styles.heroTitle}>{missionLogs.length} eventos registrados</Text>

          <Text style={styles.heroText}>
            Histórico navegável de comunicações, decisões, tarefas, alertas e
            eventos importantes da missão.
          </Text>
        ...
        <Card style={styles.timelineCard}>
          {missionLogs.map((log, index) => {
            ...
            const isLast = index === missionLogs.length - 1
              ...
              <View key={log.id} style={styles.timelineItem}>
                ...
                <View style={styles.timelineContent}>
                  ...
                  <View style={styles.responsibleRow}>
                    ...
                    <Text style={styles.responsibleText}>{log.responsible}</Text>
                  </View>
                </View>
              </View>
            )
...
function getLogConfig(type: MissionLogType): {
  label: string
